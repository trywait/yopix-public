...
export default function Test() {
  ...
  const handleRemoveBackground = async () => {
    ...
    try {
      ...
      // Set up configuration options with debug and progress
      const config = {
        debug: true,
        progress: (progress) => {
          addDebugMessage(`Progress: ${Math.round(progress * 100)}%`);
        },
        // Use proxy API to bypass potential CORS issues
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
        <p>This is a test page for isolating and fixing the background removal functionality.</p>
      </div>
